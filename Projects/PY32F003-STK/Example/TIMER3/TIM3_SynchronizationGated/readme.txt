...
逻辑分析仪可以看到，当TIM1_CH1(PA3)输出高电平时，LED电平翻转；当TIM1_CH1
(PA3)输出低电平时，LED电平不翻转
...
and using a logic analyzer, it can be observed that when TIM1_CH1 (PA3) 
outputs a high level, the LED toggles; when TIM1_CH1 (PA3) outputs 
a low level, the LED remains unchanged.
================================================================================
测试环境：
测试用板：PY32F003_STK
MDK版本： 5.28
...
使用步骤：
1. 下载并运行程序；
2. 通过逻分查看，当TIM1_CH1(PA3)输出高电平时，LED电平翻转；当TIM1_CH1
(PA3)输出低电平时，LED电平不翻转。

Example execution steps:
1.Download and run the program.
2.Use a logic analyzer to observe that when TIM1_CH1 (PA3) outputs a high 
level, the LED toggles; when TIM1_CH1 (PA3) outputs a low level, 
