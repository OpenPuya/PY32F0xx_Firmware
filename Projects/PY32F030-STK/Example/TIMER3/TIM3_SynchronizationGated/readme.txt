...
逻辑分析仪可以看到，当TIM1_CH1(PA8)输出高电平时，LED翻转；当TIM1_CH1
(PA8)输出低电平时，LED不翻转。
...
outputs a high level, the LED toggles; when TIM1_CH1 (PA8) outputs 
...
使用步骤：
1. 下载并运行程序；
2. 通过逻分查看，当TIM1_CH1(PA8)输出高电平时，LED翻转；当TIM1_CH1
(PA8)输出低电平时，LED不翻转。

Example execution steps:
1.Download and run the program.
2.Use a logic analyzer to observe that when TIM1_CH1 (PA8) outputs a high 
level, the LED toggles; when TIM1_CH1 (PA8) outputs a low level, 
