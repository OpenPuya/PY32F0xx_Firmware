...
{
  FLASH->SR |= FLASH_SR_EOP;                               // Reset FLASH_EOP
  FLASH->CR |= FLASH_CR_UPER;                              // Mass Erase Enabled
  FLASH->CR |= FLASH_CR_EOPIE;
  M32(0x1FFF0D00) = 0xFF;
  __asm("DSB");
  ...
  FLASH->CR &= ~FLASH_CR_UPER;                             // Mass Erase Disabled
  FLASH->CR &= ~FLASH_CR_EOPIE;                           // Reset FLASH_EOPIE
...
int ProgramPage(unsigned long adr, unsigned long sz, unsigned char *buf)
{
  ...
  for (u8 i = 0; i < 32; i++)
  {
    M32(0x1FFF0D00 + i * 4) = *((u32 *)(buf + i * 4));
    if (i == 30)
    {
      FLASH->CR |= FLASH_CR_UPGSTRT;
    }
  }
  __asm("DSB");
...
void InitRccAndFlashParam(void)
{
  ...
  MODIFY_REG(RCC->ICSCR, (RCC_ICSCR_HSI_FS|RCC_ICSCR_HSI_TRIM), M32(0x1FFF0F10));//HSI_24MHz
...
void UnInitRccAndFlashParam(void)
{
  ...
  case RCC_ICSCR_HSI_FS_2://24MHz
    MODIFY_REG(RCC->ICSCR, (RCC_ICSCR_HSI_FS|RCC_ICSCR_HSI_TRIM), M32(0x1FFF0F10));
    break;
  case (RCC_ICSCR_HSI_FS_1|RCC_ICSCR_HSI_FS_0)://22.12MHz
    MODIFY_REG(RCC->ICSCR, (RCC_ICSCR_HSI_FS|RCC_ICSCR_HSI_TRIM), M32(0x1FFF0F0C));
    break;
  case RCC_ICSCR_HSI_FS_1://16MHz
    MODIFY_REG(RCC->ICSCR, (RCC_ICSCR_HSI_FS|RCC_ICSCR_HSI_TRIM), M32(0x1FFF0F08));
    break;
  case RCC_ICSCR_HSI_FS_0://8MHz
    MODIFY_REG(RCC->ICSCR, (RCC_ICSCR_HSI_FS|RCC_ICSCR_HSI_TRIM), M32(0x1FFF0F04));
    break;
  default://4MHz
    MODIFY_REG(RCC->ICSCR, (RCC_ICSCR_HSI_FS|RCC_ICSCR_HSI_TRIM), M32(0x1FFF0F00));
    break;
  }
  while (RCC_CR_HSIRDY != READ_BIT(RCC->CR, RCC_CR_HSIRDY));
}
